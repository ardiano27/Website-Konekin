...
$project_sql = "
    SELECT 
        ...
        c.progress_percentage as contract_progress,
        c.payment_status,
    ...
    GROUP BY p.id, c.progress_percentage, c.payment_status
";
...
if ($project['status'] === 'in_progress' || $project['status'] === 'completed') {
    // Gunakan progress dari contract jika tersedia
    if (isset($project['contract_progress']) && $project['contract_progress'] !== null) {
        $progress = $project['contract_progress'];
    } 
    // Fallback untuk completed projects
    elseif ($project['status'] === 'completed') {
        $progress = 100;
    }
    // Fallback untuk active projects dengan milestone
    elseif ($project['milestone_count'] > 0) {
                ...
                <div class="col-lg-4">
                    ...
                    <?php if (in_array($project['status'], ['in_progress', 'completed']) && $progress > 0): ?>
                        <div class="content-card mb-3">
                            ...
                            <!-- Informasi tambahan tentang sumber progress -->
                            <?php if (isset($project['contract_progress'])): ?>
                                <div class="alert alert-info mt-3 mb-0 p-2" style="font-size: 0.8rem;">
                                    <i class="fas fa-info-circle me-1"></i>
                                    Progress diambil dari database: <?php echo $project['contract_progress']; ?>%
                                </div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center py-4">
                ...
                <h4 class="modal-title mb-3">Hapus Proyek</h4>
                <p class="text-muted mb-4">
                    Yakin ingin menghapus proyek <strong>"<?php echo htmlspecialchars($project['title']); ?>"</strong>?<br>
                    <small class="text-danger">Tindakan ini tidak dapat dibatalkan dan semua data terkait akan dihapus permanen.</small>
                </p>
                
                <?php if (in_array($project['status'], ['in_progress', 'completed'])): ?>
                    <div class="alert alert-warning">
                        <i class="fas fa-exclamation-circle me-2"></i>
                        Proyek dengan status <strong><?php echo $status_info['label']; ?></strong> tidak dapat dihapus.
                        <?php if ($project['contract_count'] > 0): ?>
                            <br>Proyek ini memiliki <?php echo $project['contract_count']; ?> kontrak aktif.
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
            </div>
            <div class="modal-footer border-0 pt-0">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                <?php if (!in_array($project['status'], ['in_progress', 'completed'])): ?>
                    <a href="delete-project.php?id=<?php echo $project['id']; ?>&confirm=1" 
